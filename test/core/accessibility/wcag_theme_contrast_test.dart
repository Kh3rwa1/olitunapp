...
void main() {
  group('WCAG 2.2 AA Theme Contrast Compliance', () {
    test('elevated button foreground passes normal text contrast on primary green', () {
      const buttonForeground = Color(0xFF00391C);
      const buttonBackground = AppColors.primary; // #1EE088
      final ratio = WcagAudit.contrastRatio(buttonForeground, buttonBackground);
      expect(
        ratio,
    ...
    test('dark theme primary text passes normal text contrast on dark background', () {
      ...
      expect(
        ...
        greaterThanOrEqualTo(WcagAudit.normalTextContrast),
        reason:
            'Dark theme primary text ($ratio:1) must meet ≥ 4.5:1',
      );
    });

    test('light theme primary text passes normal text contrast on light background', () {
      ...
      expect(
        ...
        greaterThanOrEqualTo(WcagAudit.normalTextContrast),
        reason:
            'Light theme primary text ($ratio:1) must meet ≥ 4.5:1',
      );
    });
