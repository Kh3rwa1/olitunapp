    ...
    return statsAsync.when(
      ...
      data: (stats) {
        ...
        if (levelIndex == 0) {
          ...
          final lessonsRatio = (stats.completedLessons.length / 3.0).clamp(0.0, 1.0);
          levelProgress = (progressRatio + lessonsRatio) / 2.0;

          if (stats.completedLessons.length < 3) {
            levelRequirements.add('Complete ${3 - stats.completedLessons.length} more lessons');
          }
          if (stats.overallProgress < 0.20) {
            levelRequirements.add('Reach 20% overall progress (current: ${(stats.overallProgress * 100).round()}%)');
          }
        } else if (levelIndex == 1) {
          nextLevelName = 'Advanced';
          final progressRatio = (stats.overallProgress / 0.50).clamp(0.0, 1.0);
          final lessonsRatio = (stats.completedLessons.length / 10.0).clamp(0.0, 1.0);
          levelProgress = (progressRatio + lessonsRatio) / 2.0;

          if (stats.completedLessons.length < 10) {
            levelRequirements.add('Complete ${10 - stats.completedLessons.length} more lessons');
          }
          if (stats.overallProgress < 0.50) {
            levelRequirements.add('Reach 50% overall progress (current: ${(stats.overallProgress * 100).round()}%)');
          }
        } else if (levelIndex == 2) {
          nextLevelName = 'Master';
          final progressRatio = (stats.overallProgress / 0.75).clamp(0.0, 1.0);
          final lessonsRatio = (stats.completedLessons.length / 20.0).clamp(0.0, 1.0);
          levelProgress = (progressRatio + lessonsRatio) / 2.0;

          if (stats.completedLessons.length < 20) {
            levelRequirements.add('Complete ${20 - stats.completedLessons.length} more lessons');
          }
          if (stats.overallProgress < 0.75) {
            levelRequirements.add('Reach 75% overall progress (current: ${(stats.overallProgress * 100).round()}%)');
          }
        } else {
          nextLevelName = 'Santali Sage / Guru';
          final progressRatio = stats.overallProgress;
          final lettersRatio = (stats.practicedLetters.length / 30.0).clamp(0.0, 1.0);
          ...
          if (stats.practicedLetters.length < 30) {
            levelRequirements.add('Practice ${30 - stats.practicedLetters.length} more letters');
          }
        ...
        final badges = [
          _BadgeProgress(
            ...
            targetText: 'Practice 20+ letters (${stats.practicedLetters.length}/20)',
          ...
          _BadgeProgress(
            ...
            targetText: 'Complete 3 lessons (${stats.completedLessons.length}/3)',
          ...
          _BadgeProgress(
            ...
            targetText: 'Reach 40% overall progress (${(stats.overallProgress * 100).round()}%/40%)',
          ...
          _BadgeProgress(
            ...
            targetText: 'Practice for 10 minutes (${stats.totalLearningMinutes}/10 mins)',
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      ...levelRequirements.map((req) => Padding(
