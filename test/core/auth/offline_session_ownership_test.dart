...
void main() {
  ...
  setUp(() async {
    SharedPreferences.setMockInitialValues({
      ...
      'olitun_web_session_ts': DateTime.now().subtract(const Duration(days: 2)).millisecondsSinceEpoch,
    });
    ...
    service = AppwriteAuthService.forTesting(
      client: client, account: account, functions: _Functions(),
      prefs: prefs, isWebOverride: true,
    );
  });

  test('expired web metadata plus offline validation preserves progress owner', () async {
    when(() => account.getSession(sessionId: 'current')).thenThrow(TimeoutException('offline'));
  ...
  test('401 invalidates credentials without redirecting pending progress to guest', () async {
  ...
  test('a validated different cookie creates a new owner incarnation', () async {
