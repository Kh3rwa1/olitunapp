...
void main() {
  for (final native in [true, false]) {
    ...
    testWidgets(
      '$format waits for load and frees a mounted ad only after its view leaves',
      (tester) async {
        ...
        if (native) {
          service.firstNative = first as TestNative;
        } else {
          service.firstBanner = first as TestBanner;
        }
        ...
        final load = native
            ? service.nativeLoads.single
            : service.bannerLoads.single;
        load.complete();
        await tester.pump();
    ...
    testWidgets(
      '$format clears loaded state and rejects stale callbacks on refresh',
      (tester) async {
        final service = ControlledAdService();
        final runtime = ControlledAdRuntime();
        final harness = await AdHarness.mount(
          tester,
          service,
          child: child,
          runtime: runtime,
        ...
        final pending = native
            ? service.nativeLoads.last
            : service.bannerLoads.last;
    ...
    testWidgets('$format ignores a load callback after widget disposal', (
      tester,
    ) async {
      final service = ControlledAdService();
      final runtime = ControlledAdRuntime();
  ...
  testWidgets(
    'a replacement native identity creates a fresh platform-view subtree',
    (tester) async {
      final service = ControlledAdService();
      final runtime = ControlledAdRuntime();
      await AdHarness.mount(
        tester,
        service,
  ...
  testWidgets(
    'obsolete banner sizing cannot create an ad after orientation changes',
    (tester) async {
