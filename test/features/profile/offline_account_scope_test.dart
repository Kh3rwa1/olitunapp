...
const _stats = UserStatsEntity(
  practicedLetters: {'a'}, completedLessons: {'lesson-a'}, quizHistory: {},
  categoryMastery: {}, totalLearningMinutes: 5, lastActiveDate: '',
  currentStreak: 0, totalStars: 10,
);
...
void main() {
  ...
  setUp(() async {
    ...
    auth = _Auth();
    when(auth.getCurrentUser).thenAnswer((_) async => const Left(NetworkFailure()));
    when(auth.isLoggedIn).thenAnswer((_) async => const Left(NetworkFailure()));
    when(auth.getUserPrefs).thenAnswer((_) async => const Left(NetworkFailure()));
    when(() => auth.updateUserPrefs(any())).thenAnswer((_) async => const Right(null));
    repo = ProfileRepositoryImpl(auth, prefs);
  });

  test('authenticated offline writes survive repository and preference restart, then sync', () async {
    ...
    when(auth.getUserPrefs).thenAnswer((_) async => const Right({'unrelated': 'keep'}));
    expect((await repo.syncPendingStats()).isRight(), isTrue);
    final uploaded = verify(() => auth.updateUserPrefs(captureAny())).captured.single
  ...
  test('transient auth/server failure never redirects data to guest', () async {
    await _login(prefs, 'a');
    when(auth.getUserPrefs).thenAnswer((_) async => const Left(ServerFailure(message: '503')));
  ...
  test('legacy active session with unknown owner fails closed, not guest', () async {
  ...
  test('login never imports guest/legacy data; switching preserves separate pending flags', () async {
    await repo.updateUserStats(_stats);
    await _login(prefs, 'a');
    expect((await repo.getUserStats()).getRight().toNullable()!.totalStars, 0);
    ...
    await _login(prefs, 'a');
    expect((await repo.getUserStats()).getRight().toNullable()!.totalStars, 20);
  });

  test('global clean flag cannot suppress missing per-account pending flag', () async {
    await _login(prefs, 'a');
  ...
  test('offline reset stays account-scoped and pending across reconnect', () async {
    await _login(prefs, 'a');
    await repo.updateUserStats(_stats);
    ...
    expect(prefs.getBool('is_stats_synced_a'), isFalse);
    when(auth.getUserPrefs).thenAnswer((_) async => Right({'user_progress_data': _encode(_stats)}));
    await repo.syncPendingStats();
    final stored = UserStatsModel.fromJson(jsonDecode(prefs.getString('user_stats_a')!));
  ...
  for (final operation in ['get', 'update', 'reset', 'sync']) {
    for (final fails in [false, true]) {
      test('$operation ignores late cloud ${fails ? 'exception' : 'response'} after account switch', () async {
        await _login(prefs, 'a');
        await prefs.setString('user_stats_a', _encode(_stats));
        ...
        switch (operation) {
          case 'get': pending = repo.getUserStats(); break;
          case 'update': pending = repo.updateUserStats(_stats); break;
          case 'reset': pending = repo.resetUserStats(); break;
          default: pending = repo.syncPendingStats();
        }
        ...
        if (fails) {
          ...
          cloud.complete(Right({'user_progress_data': _encode(_stats.copyWith(totalStars: 99))}));
        }
  ...
  test('late upload success cannot mark new account clean', () async {
    ...
    when(() => auth.updateUserPrefs(any())).thenAnswer((_) {
      started.complete(); return upload.future;
    });
  ...
  test('A -> logout -> A invalidates in-flight and queued operations', () async {
    await _login(prefs, 'a');
    final started = Completer<void>();
    final cloud = Completer<Either<Failure, Map<String, dynamic>>>();
    when(auth.getUserPrefs).thenAnswer((_) {
      started.complete(); return cloud.future;
    });
