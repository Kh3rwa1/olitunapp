...
void main() {
  ...
  testWidgets('private audio renews URI, preserves position, and stops on denial', (tester) async {
    ...
    when(() => player.setAudioSource(any(), initialPosition: any(named: 'initialPosition'))).thenAnswer((invocation) async {
    ...
    final media = AuthorizedMediaService(
      endpoint: 'https://media.example.test/v1', projectId: 'test', clock: () => now,
      execute: (_) async {
        requests++;
        ...
        return {
          ...
          'url': 'https://media.example.test/v1/storage/buckets/paid_media/files/file/view?project=test&token=lease$requests',
    ...
    expect(await playback.play('appwrite-storage://paid_media/file?lessonId=lesson'), true);
    expect(requests, 1);
    ...
    expect((loaded.last as UriAudioSource).uri.queryParameters['token'], 'lease2');
    denied = true;
    await playback.resume();
    expect(playback.active, false);
    expect(loaded.length, 2, reason: 'No stale source fallback after revocation');
  ...
  test('failed stop cleanup cannot leak a private resolution error', () async {
    ...
    final media = AuthorizedMediaService(
      endpoint: 'https://media.example.test/v1', projectId: 'test',
      execute: (_) async => throw const MediaAccessException(),
    );
    final playback = PrivateAudioPlayback(player, media);
    expect(await playback.play('appwrite-storage://paid_media/file?lessonId=lesson'), false);
    expect(playback.active, false);
    verify(player.stop).called(1);
    playback.cancel();
  });
