...
void main() {
  ...
  for (final disposeEarly in [false, true]) {
    test('delayed prefetch failures are observed; disposed=$disposeEarly', () async {
      final repository = _ContentRepository();
      final pending = <ContentKind, Completer<Either<Failure, List<ContentItem>>>>{};
      for (final kind in kinds) {
        pending[kind] = Completer<Either<Failure, List<ContentItem>>>();
        when(() => repository.list(kind)).thenAnswer((_) => pending[kind]!.future);
      }
      ...
      if (!disposeEarly) {
        for (final kind in kinds) {
          expect(container.read(contentListProvider((kind, null))).hasError, isTrue);
        }
      }
    });
  }

  testWidgets('content guard exposes error and retry, not empty success', (tester) async {
    var retried = false;
