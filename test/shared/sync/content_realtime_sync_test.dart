...
void main() {
  ...
  setUpAll(() async {
    ...
    tempDir = await Directory.systemTemp.createTemp('content_realtime_sync_test_');
  ...
  setUp(() async {
    ...
    when(() => mockContentRepository.cachedList(any(), categoryId: any(named: 'categoryId')))
        .thenAnswer((_) async => const Right([]));
    when(() => mockContentRepository.list(any(), categoryId: any(named: 'categoryId')))
        .thenAnswer((_) async => const Right([]));
    ...
    when(() => mockCategoryRepository.getCategories())
        .thenAnswer((_) async => const Right([]));
    when(() => mockAppwriteDbService.listDocuments(any(), queries: any(named: 'queries')))
        .thenAnswer((_) async => []);
  ...
  test('subscribes to all 7 content collection channels on initialize', () {
    ...
    expect(channels, contains('databases.olitun_db.collections.words.documents'));
    expect(channels, contains('databases.olitun_db.collections.sentences.documents'));
    expect(channels, contains('databases.olitun_db.collections.lessons.documents'));
    expect(channels, contains('databases.olitun_db.collections.letters.documents'));
    expect(channels, contains('databases.olitun_db.collections.numbers.documents'));
    expect(channels, contains('databases.olitun_db.collections.rhymes.documents'));
    expect(channels, contains('databases.olitun_db.collections.categories.documents'));

    sync.dispose();
  });

  test('incoming word message evicts cache and invalidates providers', () async {
    ...
    await CacheService.set('content_list_word_all', [{'id': 'w_1'}]);
    await CacheService.set('content_list_word_cat_vocab', [{'id': 'w_1'}]);
    ...
    expect(await CacheService.get<dynamic>('content_list_word_all', (d) => d), isNull);
    expect(await CacheService.get<dynamic>('content_list_word_cat_vocab', (d) => d), isNull);
    expect(await CacheService.get<dynamic>('content_item_word_w_1', (d) => d), isNull);
  ...
  test('incoming lesson message evicts lesson cache and invalidates learner lessons', () async {
    ContentKind? invalidatedKind;
    String? invalidatedItemId;
    String? invalidatedCategoryId;
    ...
    await CacheService.set('content_list_lesson_all', [{'id': 'lesson_greet_1'}]);
    await CacheService.set('content_list_lesson_cat_phrases', [{'id': 'lesson_greet_1'}]);
    await CacheService.set('content_item_lesson_lesson_greet_1', {'id': 'lesson_greet_1'});

    final message = RealtimeMessage(
      events: ['databases.olitun_db.collections.lessons.documents.lesson_greet_1.update'],
      payload: {'\$id': 'lesson_greet_1', 'categoryId': 'cat_phrases', 'titleLatin': 'Meeting People'},
    ...
    expect(await CacheService.get<dynamic>('content_list_lesson_all', (d) => d), isNull);
    expect(await CacheService.get<dynamic>('content_list_lesson_cat_phrases', (d) => d), isNull);
    expect(await CacheService.get<dynamic>('content_item_lesson_lesson_greet_1', (d) => d), isNull);
  ...
  test('incoming category message evicts category cache and invalidates categoryNotifierProvider', () async {
    final sync = ContentRealtimeSync(
      ref: testRef,
      networkInfo: mockNetworkInfo,
      realtimeOverride: fakeRealtime,
    ...
    await CacheService.set('cached_categories', [{'id': 'cat_vocab'}]);

    final message = RealtimeMessage(
      events: ['databases.olitun_db.collections.categories.documents.cat_vocab.update'],
    ...
    expect(await CacheService.get<dynamic>('cached_categories', (d) => d), isNull);
