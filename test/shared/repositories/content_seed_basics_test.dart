...
void main() {
  ...
  test(
    'offline list and individual detail reads use the real bundled catalog',
    () async {
      when(() => network.isConnected).thenAnswer((_) async => false);
      for (final entry in counts.entries) {
        final result = await repository.list(
          entry.key,
          categoryId: 'legacy_category',
        );
        result.fold(
          (failure) => fail('$failure'),
          (items) => expect(items.length, entry.value),
        );
      }
  ...
  test(
    'newer cached content overrides a bundled ID without duplication',
    () async {
      final items = await ContentSeedLoader.loadBundledSeedItems(
        ContentKind.number,
        null,
      ...
      expect(
        await CacheService.set('content_list_number_all', [updated.toJson()]),
        isTrue,
      );
  ...
  test('unknown IDs still return a failure', () async {
    when(() => network.isConnected).thenAnswer((_) async => false);
    expect(
      (await repository.get(ContentKind.number, 'missing-number')).isLeft(),
      isTrue,
    );
    expect(
  ...
  test(
    'bundling read-only basics does not weaken tracing write validation',
    () async {
      for (final kind in counts.keys) {
        ...
        for (final item in items) {
          ...
          expect(
            () => ContentItem.validate(kind, item.tracing),
  ...
  test(
    'bundled lessons load comprehensively including greetings and clean text',
    () async {
