...
// versioned via [schemaVersion]. Format v2:
//   {"schemaVersion": 2, "items": {itemId: <MemoryItemState.toMap()>}}
// Legacy v1 (flat itemId -> state, no version key) is migrated on load
// and rewritten in v2 on the next persist — learner progress survives
...
class ReviewStore {
  ...
  ReviewStore._(this._prefs, this._states);

  static Future<ReviewStore> load(SharedPreferences prefs) async {
    final raw = prefs.getString(storageKey);
    if (raw == null || raw.isEmpty) return ReviewStore._(prefs, {});
    try {
      ...
      // rewritten as v2 on the next persist.
      ...
      return ReviewStore._(prefs, states);
    } catch (_) {
  ...
  List<MemoryItemState> due(DateTime now, {int limit = 20}) =>
      MemoryScheduler.selectDue(_states.values, now, limit: limit);
