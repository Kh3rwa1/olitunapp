...
void main() {
  group('TypingPracticeController', () {
    ...
    setUp(() async {
      ...
      when(() => mockUserStatsNotifier.recordPracticeCompletion(
    ...
    test('1b. Initial state has needsDigits true when target contains digits', () {
      ...
      final container = createContainer();
      final state = container.read(typingPracticeControllerProvider(argsWithDigits));
      expect(state.needsDigits, isTrue);
    });

    test('2. startPractice transitions state to typing', () {
      final container = createContainer();
      container.read(typingPracticeControllerProvider(args).notifier).startPractice();
    ...
    test('3. appendChar appends correct characters correctly', () {
      ...
      container.read(typingPracticeControllerProvider(args).notifier).appendChar('\u1C5A');
      ...
      container.read(typingPracticeControllerProvider(args).notifier).appendChar('\u1C5B');
      state = container.read(typingPracticeControllerProvider(args));
      expect(state.typedSoFar, equals('\u1C5A\u1C5B'));
    });

    test('4. appendChar ignores inputs not in valid Ol Chiki / Danda range', () {
      final container = createContainer();
      container.read(typingPracticeControllerProvider(args).notifier).startPractice();

      // Type non-Ol-Chiki char like 'x'
      container.read(typingPracticeControllerProvider(args).notifier).appendChar('x');

      final state = container.read(typingPracticeControllerProvider(args));
      expect(state.typedSoFar, isEmpty);
      expect(state.attemptsTotal, equals(0)); // Rejection filter ignores it completely
    });

    test('5. appendChar handles incorrect character and triggers hint after 3 consecutive wrong attempts at same position', () {
      final container = createContainer();
      container.read(typingPracticeControllerProvider(args).notifier).startPractice();
      ...
      container.read(typingPracticeControllerProvider(args).notifier).appendChar('\u1C5D');
      var state = container.read(typingPracticeControllerProvider(args));
      expect(state.typedSoFar, equals('\u1C5A'));
    ...
    test('7. deleteLastChar deletes last char and resets wrongAtPosition', () {
      ...
      container.read(typingPracticeControllerProvider(args).notifier).appendChar('\u1C5A');
      container.read(typingPracticeControllerProvider(args).notifier).appendChar('\u1C5D'); // Wrong char, wrongAtPosition = 1

      container.read(typingPracticeControllerProvider(args).notifier).deleteLastChar();
    ...
    test('8. revealAndContinue sets typedSoFar to target, sets hint to true, and completes typing', () {
      final container = createContainer();
      container.read(typingPracticeControllerProvider(args).notifier).startPractice();

      container.read(typingPracticeControllerProvider(args).notifier).revealAndContinue();
      ...
      verify(() => mockUserStatsNotifier.recordPracticeCompletion(
    ...
    test('9. tryAgain resets practice state but preserves hasAwardedStars and avoids duplicate recording', () {
      final container = createContainer();
      container.read(typingPracticeControllerProvider(args).notifier).startPractice();
      ...
      container.read(typingPracticeControllerProvider(args).notifier).appendChar('\u1C5A');
      container.read(typingPracticeControllerProvider(args).notifier).appendChar('\u1C5B');
      container.read(typingPracticeControllerProvider(args).notifier).appendChar('\u1C5C');
      ...
      // Tap "Try again"
      container.read(typingPracticeControllerProvider(args).notifier).tryAgain();
      state = container.read(typingPracticeControllerProvider(args));
      ...
      verifyNever(() => mockUserStatsNotifier.recordPracticeCompletion(
            contentId: any(named: 'contentId'),
            contentType: any(named: 'contentType'),
            practiceMode: any(named: 'practiceMode'),
            attempts: any(named: 'attempts'),
    ...
    test('10. markCelebrationDone transitions phase from complete to done', () {
      ...
      container.read(typingPracticeControllerProvider(args).notifier).markCelebrationDone();
      state = container.read(typingPracticeControllerProvider(args));
