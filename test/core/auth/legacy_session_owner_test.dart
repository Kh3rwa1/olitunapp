...
void main() {
  ...
  setUp(() async {
    SharedPreferences.setMockInitialValues({
      ...
      'user_progress_data': 'unowned legacy progress must not become guest data',
    });
  ...
  test('a server-validated session can resolve a legacy owner after expiry', () async {
  ...
  test('web validation during sign-in cannot clear or adopt the old cookie', () async {
    final started = Completer<void>();
    final created = Completer<models.Session>();
    final oldSession = _Session();
    when(() => oldSession.userId).thenReturn('alice');
    when(() => account.getSession(sessionId: 'current')).thenAnswer((_) async => oldSession);
