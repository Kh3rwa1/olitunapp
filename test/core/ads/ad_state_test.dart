...
void main() {
  group('AdState and AdStateNotifier Tests', () {
    test('default state requires ads for free users and allows initial interstitial/rewarded', () {
      const state = AdState();
    ...
    test('AdStateNotifier records impressions and errors accurately', () {
      ...
      expect(container.read(adStateProvider).consentStatus, equals(ConsentStatus.obtained));

      notifier.recordImpression('banner', 'home_bottom');
      expect(container.read(adStateProvider).adImpressionCounts['banner_home_bottom'], equals(1));

      notifier.recordError('banner');
      expect(container.read(adStateProvider).adLoadErrors['banner'], equals(1));

      notifier.resetErrors('banner');
      expect(container.read(adStateProvider).adLoadErrors.containsKey('banner'), isFalse);
