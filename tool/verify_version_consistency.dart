...
void main() {
  ...
  final versionMatch = RegExp(r'^version:\s*([0-9]+\.[0-9]+\.[0-9]+)(\+[0-9]+)?', multiLine: true).firstMatch(pubspecContent);
  ...
  if (changelogFile.existsSync()) {
    ...
    final changelogMatch = RegExp(r'##\s*\[([0-9]+\.[0-9]+\.[0-9]+)\]').firstMatch(changelogContent);
    if (changelogMatch != null) {
      ...
      if (pubspecVersion != changelogVersion) {
        stderr.writeln('❌ Version Mismatch! pubspec.yaml ($pubspecVersion) != CHANGELOG.md ($changelogVersion)');
        exit(1);
      }
    }
  }
  ...
  if (manifestFile.existsSync()) {
    ...
    final manifestMatch = RegExp(r'"\."\s*:\s*"([0-9]+\.[0-9]+\.[0-9]+)"').firstMatch(manifestContent);
    if (manifestMatch != null) {
      ...
      if (pubspecVersion != manifestVersion) {
        stderr.writeln('❌ Version Mismatch! pubspec.yaml ($pubspecVersion) != .release-please-manifest.json ($manifestVersion)');
        exit(1);
      }
    }
  }
  ...
  final gitTag = Platform.environment['GITHUB_REF_NAME'] ?? Platform.environment['TAG_NAME'];
  if (gitTag != null && gitTag.startsWith('v')) {
    final tagVersion = gitTag.substring(1);
    stdout.writeln('🔖 Git Tag version: $tagVersion');
    if (pubspecVersion != tagVersion) {
      stderr.writeln('❌ Version Mismatch! pubspec.yaml ($pubspecVersion) != Git Tag ($tagVersion)');
      exit(1);
    }
  }
