...
class _Client extends Mock implements Client {
  _Client() {
    when(() => setSession(any())).thenReturn(this);
  }
}
...
void main() {
  ...
  for (final path in ['email', 'otp', 'anonymous', 'oauth']) {
    test(
      '$path persists session owner before a profile fetch can fail',
      () async {
        when(
        ...
        switch (path) {
          case 'email':
            ...
            break;
          case 'otp':
            await service.verifyOtp(userId: 'a', secret: 'test-otp');
            break;
          case 'anonymous':
            await service.signInAnonymously();
            break;
          default:
            expect(await service.exchangeOAuthToken('a', 'test-otp'), isTrue);
        }
  ...
  test(
    'used repository logout path clears identity offline and prevents cookie resurrection',
    () async {
      await AccountScope.capture(prefs).identify('a');
      await prefs.setBool(SessionPersistence.hasLocalSessionKey, true);
      await prefs.setString(
        SessionPersistence.webSessionSecretKey,
        'test-only',
      );
      ...
      final repo = AuthRepositoryImpl(
        remoteDataSource: AuthRemoteDataSourceImpl(service),
        networkInfo: network,
      );
  ...
  test(
    'legacy session validation learns owner; transient validation keeps it',
    () async {
      await prefs.setBool(SessionPersistence.hasLocalSessionKey, true);
      when(
  ...
  test(
    'late current-session validation cannot restore signed-out identity',
    () async {
      await prefs.setBool(SessionPersistence.hasLocalSessionKey, true);
      await AccountScope.capture(prefs).identify('a');
  ...
  test('SDK dispatch rechecks scope after connectivity awaits', () async {
    ...
    final guardedService = AppwriteAuthService.forTesting(
      ...
      prefs: prefs,
      isWebOverride: false,
      connectivity: () => connectivity.future,
    );
  ...
  test(
    'session mutation waits for an already dispatched scoped request',
    () async {
      await AccountScope.capture(prefs).identify('a');
      final scope = AccountScope.capture(prefs);
      final started = Completer<void>();
      final response = Completer<models.Preferences>();
      when(account.getPrefs).thenAnswer((_) {
        started.complete();
        return response.future;
      });
      when(account.createAnonymousSession).thenAnswer((_) async => session);
  ...
  test(
    'validation during login cannot bind the previous SDK account',
    () async {
      await AccountScope.capture(prefs).identify('a');
      await prefs.setBool(SessionPersistence.hasLocalSessionKey, true);
      final started = Completer<void>();
  ...
  test(
    'corrupt metadata and preference clear/relogin fail closed for old work',
    () async {
