...
LessonEntity scopeLessonMedia(LessonEntity lesson) {
  String? scope(String? value) => value == null
      ? null
      : PrivateMediaReference.scope(value, lesson.id);
  Map<String, dynamic>? scopeData(Map<String, dynamic>? data) => data == null
      ? null
      : Map<String, dynamic>.from(scopeLessonMediaValue(data, lesson.id) as Map);
  return LessonEntity(
    id: lesson.id,
    ...
    blocks: lesson.blocks.map((block) => LessonBlockEntity(
      type: block.type,
